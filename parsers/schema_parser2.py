...
- Only converts to "date" if JSON explicitly contains "format": "date"
...
class SchemaParser2:
    ...
    def parse_from_file(filepath: str) -> Schema:
        ...
        return SchemaParser2.parse_from_dict(schema_dict)
    ...
    def parse_from_dict(schema_dict: Dict[str, Any], name: str = "root") -> Schema:
        ...
        for field_name, field_def in properties.items():
            field = SchemaParser2._parse_field(
                field_name, 
                field_def, 
    ...
    def parse_multiple_from_file(filepath: str) -> Dict[str, Schema]:
        ...
        for schema_name, schema_dict in schemas_dict.items():
            result[schema_name] = SchemaParser2.parse_from_dict(schema_dict, name=schema_name)
    ...
    def _parse_field(name: str, definition: Dict[str, Any], is_required: bool = True) -> Field:
        ...
        if field_type == 'object':
            nested_schema = SchemaParser2.parse_from_dict(definition, name=name)
            return Field(
                name=name,
        ...
        # Handle arrays
            ...
            array_item_schema = SchemaParser2.parse_from_dict(items_def, name=f"{name}_item")
            return Field(
                name=name,
                field_type=field_type,
                is_required=is_required,
        ...
        # Handle basic types
            ...
            # Only convert to "date" if JSON explicitly specifies "format": "date"
            if field_type == 'string' and definition.get('format') == 'date':
                field_type = 'date'
            
            # Respect the type field exactly - no auto-conversion to longstring
            return Field(
                name=name,
                field_type=field_type,
                is_required=is_required
